...
# ⚡ MYSFTP (v2.1.2 — Real-Time Live Streaming Terminal Core)

**Next-Generation Multi-Platform SFTP Client & SSH Termius Suite**  
*Engineered for Public Release with Luxury Dark Gold Aesthetics, Real-Time ReadableStream Terminal & Universal Tar.Gz Streaming by **ZellRayy***
...
## 🌟 Fitur Unggulan (v2.1.2)

### 💻 1. Windows Desktop Suite (`MYSFTP-v2.1.2-Setup.exe`)
* **⚡ Real-Time Live Streaming Terminal (`ReadableStream`)**: Eksekusi perintah interaktif (seperti `wget -qO- bench.sh | bash`, `speedtest`, `pm2 logs`, `apt update`) dialirkan langsung byte-per-byte secara **real-time** ke layar saat skrip sedang berjalan tanpa perlu menunggu selesai.
* **💻 Stateful Terminal Directory Navigation (`cd`)**: Terminal monospace SSH melacak direktori kerja secara persisten. Navigasi folder dengan `cd folder`, `cd ..`, `cd /path` langsung mengubah lokasi kerja dan memperbarui prompt aktif (misal: `root@host:~/ProjectBot#`).
* **🛡️ RFC 8259 Compliant Control Character Encoder**: Bebas dari crash parser JSON akibat karakter kontrol terminal Linux (`\x08`, `\x07`, `\x1b`, dll.).
...
### 📱 2. Android Mobile App (`MYSFTP-v2.1.2.apk`)
...
### 🖥️ Windows PC / Laptop
1. Unduh **`MYSFTP-v2.1.2-Setup.exe`** dari [**GitHub Releases**](https://github.com/Bhuzel/MYSFTP/releases/latest).
...
### 📱 Android Mobile
1. Unduh **`MYSFTP-v2.1.2.apk`** dari [**GitHub Releases**](https://github.com/Bhuzel/MYSFTP/releases/latest).
...
---
<div align="center">
  <sub>MYSFTP v2.1.2 — Engineered with passion for speed, beauty, and developer productivity.</sub>
</div>
